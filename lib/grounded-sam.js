 ...
 */
export async function maskGarment(originalImageUrl, labelText, negativeLabelText = "") {
  ...
  try {
    ...
    // Add ~5% padding around the content bbox, clamped to image bounds.
    const padX = Math.max(1, Math.floor(bboxW * 0.05));
    const padY = Math.max(1, Math.floor(bboxH * 0.05));
    const cropLeft   = Math.max(0, minX - padX);
    const cropTop    = Math.max(0, minY - padY);
    const cropRight  = Math.min(W - 1, maxX + padX);
    const cropBottom = Math.min(H - 1, maxY + padY);
