...
async function _attemptMask(originalImageUrl, labelText, effectivePrompt, effectiveNegative, category) {
  ...
  try {
    ...
    // Default floor is 1.5% of frame. Shoes in a worn portrait shot (full-body
    // mirror selfie) are legitimately tiny — a 0.5% blob can be a real shoe — so
    // drop the floor for them. The shoe sanity checks below (max-size + bottom-
    // of-frame centroid) are the real guard against noise specks, so a low floor
    // here is safe and lets worn shoe photos through.
    const catEarly = (category || "").toLowerCase();
    const isWornShoe = SHOE_RE.test(catEarly) && (H / W) > 1.3;
    const QUALITY_FLOOR = isWornShoe ? 0.002 : 0.015;
    const componentFraction = bestCount / (ccW * ccH);
    if (componentFraction < QUALITY_FLOOR) {
      console.log(`grounded-sam: quality gate — largest component only ${(componentFraction * 100).toFixed(2)}% of frame for "${labelText}" (floor ${(QUALITY_FLOOR * 100).toFixed(1)}%), skipping thumb`);
      return null;
    }
    ...
    // A shoe blob that fills a big share of the frame is a CLOSE-UP product shot
    // (the garment IS the photo), not a full-body mirror selfie. Trust it as-is.
    // The mirror-shot sanity checks below assume the shoe is a tiny blob sitting
    // at the bottom of a tall frame — they must NOT run on close-ups, or they
    // reject perfectly good cutouts (a portrait-orientation close-up is also
    // "tall", so the old H/W > 1.3 test alone wrongly treated it as a mirror shot).
    const isCloseupShoe = SHOE_RE.test(cat) && componentFraction > 0.25;
    if (SHOE_RE.test(cat) && sourceAspectGate > 1.3 && !isCloseupShoe) {
      // Small blob in a tall frame = mirror shot. The shoe must sit near the
      // bottom; a blob centered higher up is wall/floor/body noise, not shoes.
      let sumY = 0;
      ...
      if (centroidYNorm < 0.60) {
        console.log(`grounded-sam: shoe gate — small blob (${(componentFraction * 100).toFixed(1)}%) centroid Y=${centroidYNorm.toFixed(2)} above bottom 40% for "${labelText}", skipping thumb`);
        return null;
      }
    } else if (isCloseupShoe) {
      console.log(`grounded-sam: shoe close-up — blob fills ${(componentFraction * 100).toFixed(1)}% of frame for "${labelText}", trusting cutout as-is`);
    }
    ...
    if (SHOE_RE.test(cat) && !isCloseupShoe) {
      // Only slice when the mask is leg-shaped (taller than wide). That's the
      // failure case where DINO grounded the legs along with the shoes — the
      // bottom 18% extracts just the foot. A shoe-shaped mask (wider than
      // tall) means DINO already isolated the shoes; slicing would chop the
      // bbox down to a thin sole strip and ship a near-empty thumb.
      // Close-ups (isCloseupShoe) are never sliced — the shoe fills the frame,
      // so the whole bbox is the shoe and slicing would crop it to a sliver.
      if (maskAspect >= 1.0) {
