...
export async function maskGarment(originalImageUrl, labelText, negativeLabelText = "", category = "") {
  ...
  // Apply category-aware prompt rewrites: drop color from shoe prompts so DINO
  // doesn't ground against white-veined walls, and supply a default negative
  // prompt when the caller didn't.
  const effectivePrompt   = normalizeMaskPrompt(labelText, category);
  const effectiveNegative = negativeLabelText || defaultNegativePrompt(category);
