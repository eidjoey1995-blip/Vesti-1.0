...
/**
 * whiteLightnessFail(hex, colorName) → boolean
 *
 * Secondary check for "white"-declared garments. ΔE alone is too forgiving:
 * warm-tinted marble (#d9c7b9, L≈81) sits 18 ΔE from canonical white — under
 * our 25 threshold — but visually it's a beige floor, not a white shoe.
 * Requiring L* > WHITE_L_FLOOR catches that without nuking real whites
 * (#f0f0f0 → L≈94) or cream/ivory shades (which Claude tags differently).
 *
 * Scoped to colorName === "white" — "off-white", "ivory", "cream" have looser
 * expectations and pass through. If Claude correctly tags marble as off-white
 * the user has bigger problems than this gate.
 */
const WHITE_L_FLOOR = 85;

export function whiteLightnessFail(hex, colorName) {
  const key = (colorName || "").trim().toLowerCase();
  if (key !== "white" && key !== "pure white") return false;
  const rgb = hexToRgb(hex);
  if (!rgb) return false;
  const [L] = rgbToLab(rgb);
  return L < WHITE_L_FLOOR;
}
