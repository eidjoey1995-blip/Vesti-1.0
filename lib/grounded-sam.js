...
export async function maskGarment(originalImageUrl, labelText, negativeLabelText = "", category = "") {
  ...
  if (prediction?.status !== "succeeded") {
    ...
    if (!resolved) {
      console.log(`grounded-sam: polling timed out after 10 s for "${labelText}"`);
      return null;
    }
  }
  ...
  try {
    ...
    if (aspectRatio > 1.4 && category) {
      const cat = category.toLowerCase();
      let topPct = 0, bottomPct = 1;

      if (/(^|\s)(top|tops|t-shirt|tee|shirt|blouse|sweater|hoodie|knit)/.test(cat)) {
        topPct = 0; bottomPct = 0.55;
      } else if (/(^|\s)(bottom|bottoms|pants|chinos|trousers|jeans|shorts|skirt)/.test(cat)) {
        topPct = 0.35; bottomPct = 0.92;
      } else if (/(^|\s)(shoe|shoes|sneakers|boots|sandals|loafers)/.test(cat)) {
        topPct = 0.78; bottomPct = 1.0;
      } else if (/(^|\s)(outerwear|jacket|coat|blazer)/.test(cat)) {
        topPct = 0; bottomPct = 0.70;
      } else if (/(^|\s)(suit|suits|dress|jumpsuit)/.test(cat)) {
